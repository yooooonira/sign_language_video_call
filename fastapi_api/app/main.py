...
# ==============================================
# <<< CHANGED: load_model 에 로깅 강화(클래스/피처 차원)
# ==============================================
...
def load_model():
    ...
    try:
        ...
        logger.info(
            f"TFLite loaded: in={_in_det[0]['shape']} out={_out_det[0]['shape']} path={MODEL_PATH}"
        )

        # >>> ADDED: 입출력 shape/피처차원/클래스수 로깅
        in_shape  = tuple(int(s) for s in _in_det[0]['shape'])
        out_shape = tuple(int(s) for s in _out_det[0]['shape'])
        feature_dim = int(in_shape[-1]) if len(in_shape) >= 3 else None  # (1,10,55)라면 55
        num_classes = int(out_shape[-1]) if len(out_shape) >= 1 else None

        logger.info("TFLite I/O shapes -> in=%s, out=%s", in_shape, out_shape)
...
# >>> ADDED: (10,21,2) → (1,10,55) 임시 패딩 전처리
...
# >>> ADDED: 어떤 입력이 와도 (1,10,55)로 강제 변환
def _coerce_to_1x10x55(arr: np.ndarray) -> np.ndarray:
    """
    허용 케이스와 변환:
      - (21,2)      : 단일 프레임 → 10프레임으로 타일링 → preprocess_to_55()
...
# >>> CHANGED: 시퀀스 경로도 (1,10,55)로 강제 변환
